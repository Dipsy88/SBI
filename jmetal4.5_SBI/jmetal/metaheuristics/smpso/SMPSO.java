...
/**
 * This class implements the SMPSO algorithm described in: A.J. Nebro, J.J.
 * Durillo, J. Garcia-Nieto, C.A. Coello Coello, F. Luna and E. Alba "SMPSO: A
 * New PSO-based Metaheuristic for Multi-objective Optimization". IEEE Symposium
 * on Computational Intelligence in Multicriteria Decision-Making (MCDM 2009),
 * pp: 66-73. March 2009
 */
public class SMPSO extends Algorithm {
	...
	public SMPSO(Problem problem, Vector<Double> variables, String trueParetoFront) throws FileNotFoundException {
		super(problem);
		...
		trueHypervolume_ = hy_.hypervolume(trueFront_.writeObjectivesToMatrix(), trueFront_.writeObjectivesToMatrix(),
		...
		for (int i = 0; i < swarmSize_; i++) {
			...
			for (int var = 0; var < particle.getNumberOfDecisionVariables(); var++) {
				...
				speed_[i][var] = velocityConstriction(
						constrictionCoefficient(C1, C2) * (inertiaWeight(iter, miter, wmax, wmin) * speed_[i][var]
	 ...
	 * @throws Exception
	 */
	private void mopsoMutation(int actualIteration, int totalIterations) throws Exception {
		for (int i = 0; i < particles_.size(); i++) {
			...
			// } else if (i % 3 == 1) { //particles_ mutated with a uniform mutation
			// operator
