...
public class PSO extends Algorithm {
	...
	private double velocityConstriction(double v, double[] deltaMax, double[] deltaMin, int variableIndex,
		 ...
		 * double result;
		 * 
		 * double dmax = deltaMax[variableIndex]; double dmin = deltaMin[variableIndex];
		 * 
		 * result = v;
		 * 
	...
	private void computeSpeed(int iter, int miter) throws JMException, IOException {
		...
		for (int i = 0; i < particlesSize_; i++) {
			...
			wmin = WMin_;
			/*
			 * for (int var = 0; var < particle.size(); var++) { //Computing the velocity of
			 * this particle speed_[i][var] =
			 * velocityConstriction(constrictionCoefficient(C1, C2) * (inertiaWeight(iter,
			 * miter, wmax, wmin) * speed_[i][var] + C1 * r1 * (bestParticle.getValue(var) -
			 * particle.getValue(var)) + C2 * r2 * (bestGlobal.getValue(var) -
			 * particle.getValue(var))), deltaMax_, deltaMin_, //[var], var, i); }
			 */
			C1 = 1.5;
			C2 = 1.5;
			double W = 0.9;
