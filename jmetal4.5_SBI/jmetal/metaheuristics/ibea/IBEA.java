...
public class IBEA extends Algorithm {
	...
	double calcHypervolumeIndicator(Solution p_ind_a, Solution p_ind_b, int d, double maximumValues[],
		...
		if (d == 1) {
			...
			if (a < b) {
				...
				volume = calcHypervolumeIndicator(p_ind_a, p_ind_b, d - 1, maximumValues, minimumValues) * (max - a)
						/ r;
			}
		}
	...
	public SolutionSet execute() throws Exception {
		...
		int a;
		SolutionSet returnSet = new SolutionSet(100);
		if (ranking.getSubfront(0).size() < 100) {
			int counter = 0;
			int j = 0;
			while (returnSet.size() < 100) {
				returnSet.add(ranking.getSubfront(counter).get(j));
				if (ranking.getSubfront(counter).size() == returnSet.size()) {
					counter++;
					j = 0;
				} else
					j++;
			...
			returnSet = ranking.getSubfront(0);
