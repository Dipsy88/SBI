...
/**
 * This class represents the original synchronous MOCell algorithm A description
 * of MOCell can be consulted in Nebro A. J., Durillo J.J, Luna F., Dorronsoro
 * B., Alba E. : "MOCell: A cellular genetic algorithm for multiobjective
 * optimization", International Journal of Intelligent Systems. Vol.24, No. 7
 * (July 2009), pp. 726-746
 */
public class sMOCell1 extends Algorithm {
	...
	public SolutionSet execute() throws Exception {
		...
		Comparator dominance = new DominanceComparator(), crowding = new CrowdingComparator();
		...
		while (evaluations < maxEvaluations) {
			...
			for (int ind = 0; ind < currentSolutionSet.size(); ind++) {
				...
				if (flag == 1) { // The offSpring dominates
					...
					// neighbors[ind].sort(new FitnessAndCrowdingDistanceComparator()); //Create a
					// new comparator;
