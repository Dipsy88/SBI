...
/**
 * This class implements the AbYSS algorithm. This algorithm is an adaptation of
 * the single-objective scatter search template defined by F. Glover in: F.
 * Glover. "A template for scatter search and path relinking", Lecture Notes in
 * Computer Science, Springer Verlag, 1997. AbYSS is described in: A.J. Nebro,
 * F. Luna, E. Alba, B. Dorronsoro, J.J. Durillo, A. Beham "AbYSS: Adapting
 * Scatter Search to Multiobjective Optimization." IEEE Transactions on
 * Evolutionary Computation. Vol. 12, No. 4 (August 2008), pp. 439-457
 */
public class AbYSS extends Algorithm {

	/**
	 * Stores the number of subranges in which each encodings.variable is divided.
	 * Used in the diversification method. By default it takes the value 4 (see the
	 * method <code>initParams</code>).
	 */
	...
	/**
	 * Stores the solutions provided by the subset generation method of the scatter
	 * search template
	 */
	...
	public Solution diversificationGeneration() throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < problem_.getNumberOfVariables(); i++) {
			...
			double low = problem_.getLowerLimit(i)
					+ range * (problem_.getUpperLimit(i) - problem_.getLowerLimit(i)) / numberOfSubranges_;
	 ...
	 * @throws Exception
	 */
	public void referenceSetUpdate(boolean build) throws Exception {
		if (build) { // Build a new reference set
			...
			for (int i = 0; i < solutionSet_.size(); i++) {
				...
				individual
						.setDistanceToSolutionSet(distance_.distanceToSolutionSetInSolutionSpace(individual, refSet1_));
			}
	 ...
	 * @throws Exception
	 */
	public int subSetGeneration() throws Exception {
