...
public class MOEAD_DRA extends Algorithm {
	 ...
	 *
	 */
	void initIdealPoint() throws Exception {
	 ...
	 *         the paper describing MOEA/D-DRA (CEC 09 COMPTETITION) An example is
	 *         giving in that paper for two objectives. If N = 100, then the best
	 *         solutions attenting to the weights (0,1), (1/99,98/99),
	 *         ...,(98/99,1/99), (1,0) are selected.
	 * 
	 *         Using this method result in 101 solutions instead of 100. We will
	 *         just compute 100 even distributed weights and used them. The result
	 *         is the same
	 * 
	 *         In case of more than two objectives the procedure is: 1- Select a
	 *         solution at random 2- Select the solution from the population which
	 *         have maximum distance to it (whithout considering the already
	 *         included)
	 * 
	 * 
	 * 
	...
	SolutionSet finalSelection(int n) throws JMException {
		...
		if (problem_.getNumberOfObjectives() == 2) { // subcase 1
			...
			for (int i = 0; i < n; i++) {
				...
				for (int j = 1; j < n; j++) {
					double aux = fitnessFunction(population_.get(j), intern_lambda[i]); // we are looking the best for
																						// the weight i
			...
			// create a list containing all the solutions but the selected one (only
			// references to them)
			...
			while (res.size() < n) {
				...
				while (i < candidate.size()) {
					...
					double aux = distance_value = distance_utility.distanceToSolutionSetInObjectiveSpace(next_candidate,
							res);
