 ...
 *           pp. 257-271, 1999.
 */
public class HypervolumeBACKUPforR_RAW {
...
//  static String []  algorithmNameList_ = new String[] {"CELLDE","IBEA", "MOCELL","NSGAII","SPEA2","SMPSO"};
  static String []  algorithmNameList_ = new String[] {"SMPSO"};

  static int numberCases = 165;
  
  /**
  * Constructor
  * Creates a new instance of MultiDelta
  */
  public HypervolumeBACKUPforR_RAW() {
 ...
 * @throws Exception 
  ...
  public static void main(String args[]) throws Exception {
	  for (int algorithmRun=0;algorithmRun<algorithmNameList_.length;algorithmRun++){
			...
			for (int row=1;row<101;row++){
				 for (int column=1;column<76;column++){
					  double [][] solutionFront =readFile(row, column,algorithmNameList_[algorithmRun] );
				    ...
				    */
				    ...
				    HypervolumeBACKUPforR_RAW qualityIndicator = new HypervolumeBACKUPforR_RAW();
	...
	public static double [][] readFile(int row, int column, String algorithm){		
		double [][] tempFront = new double[100][5] ;
	    double [][] trueFront;
	    int counter;
	   int i=0;
			try {
				//System.out.println("/Users/Dipesh/OneDrive/Papers/ICST2015/Result/RAW/"+numberCases+"/"+algorithm+"/"+algorithm + column + "_" + row);
	...
	//		    		break;
			    	...
			    	tempFront[i][0] =Double.parseDouble(details[0]);
			    	tempFront[i][1] =Double.parseDouble(details[1]);
			    	tempFront[i][2] =Double.parseDouble(details[2]);
			    	tempFront[i][3] =Double.parseDouble(details[3]);
			    	tempFront[i][4] =Double.parseDouble(details[4]);
			    	i++;
			    	if (i==100)
			    		break;
			    }
			...
			double [][] solutionFront = new double[i][5] ;
			for (int j=0;j<i;j++){
				solutionFront[j][0] = tempFront[j][0];
				solutionFront[j][1] = tempFront[j][1];
				solutionFront[j][2] = tempFront[j][2];
				solutionFront[j][3] = tempFront[j][3];
				solutionFront[j][4] = tempFront[j][4];
			}
	...
	public static void createFile(String algorithm) throws Exception{
		fileName = new File("/Users/Dipesh/OneDrive/Papers/ICST2015/Analysis/RAW/"+numberCases+"/Hypervolume/forR/"+algorithm+ ".txt");
