...
public class ArrayReal extends Variable {
	...
	// Dipesh
	public ArrayReal(int size, Problem problem, double[] values) {
		problem_ = problem;
		size_ = size;
		array_ = new Double[size_];

		for (int i = 0; i < size_; i++) {
			array_[i] = new Double(values[i]);
		} // for
	} // Constructor
	...
	public String toString() {
		...
		boolean contains = Arrays.asList(a).contains(4.4);
...
//	
	...
	public boolean contains(Double item){
		if(Arrays.asList(array_).contains(item))
			return true;
		return false;
	}
