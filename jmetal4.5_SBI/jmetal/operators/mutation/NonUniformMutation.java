...
public class NonUniformMutation extends Mutation {
	...
	/**
	 * perturbation_ stores the perturbation value used in the Non Uniform mutation
	 * operator
	 */
	...
	public void doMutation(double probability, Solution solution) throws JMException {
		...
		for (int var = 0; var < solution.getDecisionVariables().length; var++) {
			if (PseudoRandom.randDouble() < probability) {
				...
				if (rand <= 0.5) {
					tmp = delta(x.getUpperBound(var) - x.getValue(var), perturbation_.doubleValue());
					tmp += x.getValue(var);
				} else {
					tmp = delta(x.getLowerBound(var) - x.getValue(var), perturbation_.doubleValue());
					tmp += x.getValue(var);
				}
	...
	public Object execute(Object object) throws JMException {
		...
		if (!VALID_TYPES.contains(solution.getType().getClass())) {
			Configuration.logger_.severe(
					"NonUniformMutation.execute: the solution " + solution.getType() + "is not of the right type");
