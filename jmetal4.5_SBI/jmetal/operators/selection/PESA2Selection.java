...
public class PESA2Selection extends Selection {
	 ...
	 * 
	 * @param object
	 *            Object representing a SolutionSet. This solution set must be an
	 *            instancen <code>AdaptiveGridArchive</code>
	...
	public Object execute(Object object) throws JMException {
		try {
			...
			if (hypercube1 != hypercube2) {
				if (archive.getGrid().getLocationDensity(hypercube1) < archive.getGrid()
						.getLocationDensity(hypercube2)) {

					selected = hypercube1;

				} else if (archive.getGrid().getLocationDensity(hypercube2) < archive.getGrid()
						.getLocationDensity(hypercube1)) {
			...
			Configuration.logger_.severe("PESA2Selection.execute: ClassCastException. " + "Found" + object.getClass()
