 ...
 * solutions.
 */
public class SinglePointCrossover_Generation_DPR extends Crossover {
	/**
	 ...
	 */
	public SinglePointCrossover_Generation_DPR(HashMap<String, Object> parameters) {
		super(parameters);
		...
		try {
			if (PseudoRandom.randDouble() < probability) {
				if ((parent1.getType().getClass() == BinarySolutionType.class)
					...
					ArrayList<TestCase> caseList1 = new ArrayList<TestCase>();
					ArrayList<TestCase> caseList2 = new ArrayList<TestCase>();
					// put each variable value along with its test case
					// Map<Double,TestCase> caseWithValue1 = new HashMap<Double,
					// TestCase>();
					// Map<Double,TestCase> caseWithValue2 = new HashMap<Double,
					// TestCase>();
					...
					List<TestCase> testCaseList1 = cloner1.deepClone(parent1.getCaseList());
					List<TestCase> testCaseList2 = cloner2.deepClone(parent2.getCaseList());
					...
					for (int i = 0; i < crossoverPoint; i++) {
						...
						caseList1.add(testCaseList1.get(i));
						caseList2.add(testCaseList2.get(i));
						// }
						// caseWithValue1.put(X1.getValue(i),
						// parent1.getCaseList().get(i));
						// caseWithValue2.put(X2.getValue(i),
						// parent2.getCaseList().get(i));
					}

					for (int i = crossoverPoint; i < parent1.numberOfVariables(); i++) {
						valueX1 = X1.getValue(i);
						valueX2 = X2.getValue(i);
						caseList1.add(testCaseList2.get(i));
						caseList2.add(testCaseList1.get(i));

						while (X1List.contains(valueX2))
							valueX2 += 0.000001;
						offSpring[0].getDecisionVariables()[0].setValue(i, valueX2);
						X1List.add(valueX2);

						while (X2List.contains(valueX1))
							valueX1 += 0.000001;
