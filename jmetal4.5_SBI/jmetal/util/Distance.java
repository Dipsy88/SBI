...
public class Distance {
	...
	public double distanceBetweenSolutions(Solution solutionI, Solution solutionJ) throws JMException {
		 ...
		 * (solutionJ.getDecisionVariables() != null)) { Variable[] decisionVariableI =
		 * solutionI.getDecisionVariables(); Variable[] decisionVariableJ =
		 * solutionJ.getDecisionVariables();
		 * 
		 * double diff; //Auxiliar var //-> Calculate the Euclidean distance for (int i
		 * = 0; i < decisionVariableI.length; i++){ diff =
		 * decisionVariableI[i].getValue() - decisionVariableJ[i].getValue(); distance
		 * += Math.pow(diff,2.0); } // for } //-> Return the euclidean distance return
		 * Math.sqrt(distance);
		 */
		double distance = 0.0;
